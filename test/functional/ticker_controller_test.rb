...
require 'ticker_controller'

# Re-raise errors caught by the controller.
class TickerController; def rescue_action(e) raise e end; end

class TickerControllerTest < Test::Unit::TestCase
  fixtures :users
  
  def setup
    @controller = TickerController.new
  ...
  def test_categorized_issues
    get :categorized_issues,:issue_category=>"Exist Services Activites"
    assert_response :success
    u = User.find(2)
    [:user,:projects,:issues].each{|assigns| assert_not_nil(assigns)}
    #assert(assigns[:projects].kind_of?(Array),"projects not an array")    
    
    assert_template('categorized_issues')
  end
  
  def test_categorized_issues_with_non_existent_category
    get :categorized_issues,:issue_category=>"bad category"
    assert_response :success
    u = User.find(2)
    [:user,:projects,:issues].each{|assigns| assert_not_nil(assigns)}
    #assert(assigns[:projects].kind_of?(Array),"projects not an array")    
    assert(assigns(:issues).size == 0)
    assert_template('categorized_issues')
  end
...
class IssuesControllerTest < Test::Unit::TestCase
  ...
  def test_post_edit_with_status_and_assignee_change
    ...
    #changed from 2 to 3 since time log is now included in the journal
    assert_equal 3, j.details.size
  ...
  def test_post_edit_with_note_and_spent_time
    ...
    #changed from 0 to 1 cause time log is now included in journals
