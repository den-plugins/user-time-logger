...
def leave_entries_to_csv(leaves, projects, option)
  ...
  def highlight_allocated_class(params,summary)
    exceed_hours = params[:total_hours_on_selected].to_i > params[:max_hours].to_i
    proj_admin = (params[:project_id].to_i.eql?(summary[:project_id]) || summary[:admin])
    exceed_hours && proj_admin ? 'redFont' : ''
  end
